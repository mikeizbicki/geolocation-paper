...
%\title{Geolocating Tweets in any Language at any Location\\with Unicode Convolutional Neural Networks\\and Mixtures of Fisher-von Mises Distributions}
\title{Geolocating Tweets in any Language at any Location\\with the UnicodeCNN and MFvM distribution}
...
\begin{abstract}
    ...
    %To predict GPS locations, we introduce a novel output layer for neural networks we call the mixture of Fisher-von Mises (FvM) layer.
    To predict these GPS coordinates,
    we introduce a novel output layer using the \defn{mixture of Fisher-von Mises} (MFvM) distribution.
    The MFvM exploits the Earth's spherical geometry to improve predictions,
    whereas all previous geolocation work has ignored the Earth's geometry.
    By combining the UnicodeCNN and the MFvM,
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        %\node at (0,0) {\resizebox{0.485\textwidth}{!}{\input{img/infer/es-vosotros-hablar/0-good-zoom.pgf}}};
        %\node at (0,0) {\resizebox{0.485\textwidth}{!}{\includegraphics{img/infer/es-vosotros-hablar/0-good-zoom}}};
