...
The UnicodeCNN operates directly on the Unicode characters in the input text and requires no preprocessing or lexical analysis.
...
We use the same model to analyze English, Arabic, Japanese, and the more than 100 other languages present in our Twitter dataset.
...
Remarkably, we show that geographic information learned in one language is automatically learned in other languages as well even when that information is not directly represented in the training data.
...
%Let's try to guess where the following tweet was sent from:
    ...
    Our first example illustrates how the UnicodeCNN is able to detect subtle linguistic patterns that other models cannot detect.
...
\end{quote}
A good model should still guess this tweet was written from Kuwait despite the language change,
...
\noindent
Similar transfers of knowledge hold for many other location and language combinations.
...
Extracting words from mixed language Japanese/English text is difficult because whereas English uses spaces to delimit words,
Japanese does not use spaces and relies on context.
Special Japanese-language parsers have been developed for Japanese-only text,
