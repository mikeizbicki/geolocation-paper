...
\begin{abstract}
    ...
    but our best text mining tools work only for English.
    ...
    This paper introduces a method for analyzing highly multilingual text corpora in a simple, unified manner.
    ...
    The key to our method is a novel \defn{Unicode Convolutional Neural Network} (UnicodeCNN) that accepts any Unicode string as input.
    ...
    Due to the high-quality featuers of the UnicodeCNN,
    ...
    Previous state-of-the-art work restricted itself to predicting he country or city the tweet was sent from, and filtered out particularly ``hard'' tweets from their evaluation datasets.
...
Convolutional neural networks have previously been successfully applied to English-only text corpora \citep{zhang2015character,conneau2017very},
...
Furthermore, the UnicodeCNN is especially suited to analyzing social media text because it is naturally robust to the spelling mistakes and non-standard grammar commonly found in this domain.
...
Our UnicodeCNN significantly improves the geolocation state-of-the art in three ways.
...
\citet{han2014text} propose the most advanced multilingual system,
which first detects the language of the tweet,
...
The UnicodeCNN, in contrast, processes all languages in a unified method and seamlessly allows information learned in one language to be transferred to another language.
...
We show that the high quality features generated by the UnicodeCNN enable us to solve this more complicated prediction problem.
...
we present some simple experiments.
We use these experiments to introduce the details of the geolocation problem,
and demonstrate the qualitative ability of our UnicodeCNN model to learn complex language features.
...
%Let's try to guess where the following tweet was sent from:
    ...
    This example shows how the UnicodeCNN is able to detect subtle linguistic patterns that other models cannot detect.
    Consider the following Spanish language tweet:
\begin{quote}
    \str{Qu\'e gran ma\~nana... emocionante.  Pod\'eis ver el v\'ideo abajo en el enlace.}%
...
%The first clue to consider is the tweet's language.
...
Importantly, these patterns generalize to words not present in the original training set.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item[Example 2.]
    This example shows how the UnicodeCNN can learn information in one language and apply it to other languages.
...
So, for example, if we translate the Arabic in the previous tweet into Japanese, we get
\begin{quote}
...
\end{quote}
then we should still predict this tweet was sent from Kuwait.
Amazingly, the UnicodeCNN is able to predict this tweet was sent from Kuwait even though the Japanese word for Kuwait (\begin{CJK}{UTF8}{min}クウェート\end{CJK}) never appears in our training data.
The UnicodeCNN learned that \begin{CJK}{UTF8}{min}クウェート\end{CJK} is Japanese for Kuwait by learning that this word encodes a similar sound as the Arabic and English words for Kuwait.
Furthermore, the UnicodeCNN even understands misspellings of words it learns indirectly in this way.
If we misspell the Japanese word for Kuwait (for example as \begin{CJK}{UTF8}{min}クウェート\end{CJK}),
then the UnicodeCNN is still able to understand that the tweet should be located in Kuwait.
...
In this section, we describe our dataset, baseline comparison models, training procedures, and our experimental results.
...
The dataset contains approximately 900 million tweets written by $3.0\times10^6$ users.
...
and a country code is reserved for tweets sent from outside the border of any country (e.g.\ tweets from international waters).
The process of assigning GPS coordinates is slightly more complicated.
The Twitter API associates exact GPS coordinates with approximately 16\% of tweets in our dataset.
For the remaining tweets, user privacy settings restrict the accuracy that the tweet's location can be known to a higher level, typically the city.
We take the center of mass of these cities as the true GPS location of the tweet.
...
Our method preserves this information and results in a more difficult prediction problem.
...
We use a single machine with 16 CPUs, 64 GB of RAM, and 6 NVidia Titan x80 GPUs to train our UnicodeCNN model.
We train using the Adam optimizer \citep{kingma2014adam} with a learning rate of $5\times10^{-4}$ and a batch size of 600.
We train in using data parallelism, so that each GPU processes 100 tweets per batch, and then the gradients are averaged together.
The learning rate was set by training only on a small held-out subset of tweets,
and the batch size of 100/GPU is the largest batchsize we could fit on a GPU with the huge model.
We observed about a 5-fold speed up using all 6 GPUs,
and in total, training the large model took about 1 month with this setup.
Training the huge model seems to take about \fixme{} time.
We let it train for 2 weeks but then ran out of time.
...
Random hyperparameter search is a state-of-the-art hyperparameter tuning method that works well when there is more than one hyperparameter to tune.
...
We trained 20 versions of each model and report only the model with the best country classification accuracy.
...
Table \ref{table:results} contains the results.
