...
\begin{abstract}
    ...
    Remarkably, we further show that the UnicodeCNN can learn geographical knowledge in one language and automatically transfer that knowledge to other languages.
...
%Several recent studies have raised concern that the emphasis on English causes unfair bias in the analysis of minority communities \citep{blodgett2017racial,hargittai2018potential}.
...
Most prior work considers only English-language tweets \citep{cheng2010you,li2012towards,han2013stacking,mahmud2014home,compton2014geotagging,zhang2014geocoding,rahimi2015twitter,dredze2016geolocation,duong2014near,rahimi2017neural},
...
Their learning pipeline therefore requires special tools for lexical analysis, stemming, and other preprocessing.
Because these tools are language specific, 
it is difficult to extend these word-level approaches to the highly multilingual setting we consider.
...
%Predi``presents computational challenges, and is too fine-grained'' for effective estimation \citep{han2012geolocation}.
...
but no previous work studying Spanish language geolocation exploits these variations 
\citep{maier2014language,gonccalves2015learning,tinoco2017variation,han2014text}. 
Exploiting these variations is difficult for methods using word-level features because these methods require explicit tokenization,
and the tokenization procedure would somehow need to understand these linguistic variations.
Special purpose tokenizers for tweet analysis have been built for English \citep{o2010tweetmotif},
...
but these languages were ignored by \citep{han2014text}.
...
\begin{figure*}
    ...
    \caption{
        ...
        For the task of geolocation, we add two outputs to predict the country using the standard cross entropy loss and the GPS coordinates using a novel mixture of von Mises-Fisher distributions.
...
We evaluate three versions of the UnicodeCNN with different hyperparameter settings 
(which we call the Small, Large, and Huge models),
...
and \defn{character properties} is the generic Unicode term for other information such as the category a code point belongs to (punctuation, letter, etc.).
...
and so it would be foolish to ignore.
Our character encoder works at the code point level instead of the encoding level and exploits this information.
...
This conversion is necessary because convolutional layers require numeric inputs.
...
\begin{description}
    ...
    \item[Mixture of von Mises-Fisher.]
        ...
        Our method does not suffer from these problems because it accounts for the Earth's spherical geometry.
...
\begin{table}
    \input{img/tables/versus}
    \textbf{
    \caption{
        \label{table:results_lang}
    }
    }
\end{table}
