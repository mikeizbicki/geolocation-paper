...
\setlist[enumerate]{topsep=0.1cm,leftmargin=0.4cm,itemsep=0.1cm}
...
\iftoggle{shortpaper}{
    ...
    \newcommand{\cutt}[2]{\color{red}{#1}\color{black}}
    \newcommand{\add}[1]{\color{darkgreen}{#1}\color{black}}
}

\newcommand{\addshort}[1]{\color{darkgreen}{#1}\color{black}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\title{Geolocating Tweets in any Language at any Location
%\\using the UnicodeCNN and MvMF distribution
}
...
\begin{abstract}
    ...
    \cut{~(and only worked on tweets written in certain languages from highly populated locations)},
    we predict the exact GPS locations of tweets%
    \cut{~(and our method works on tweets written in any language sent from anywhere in the world)}.
    ...
    \cut{Remarkably, the UnicodeCNN can learn geographic knowledge in one language and automatically transfer that knowledge to other languages.}
\end{abstract}
...
\keywords{Multilingual; Geotagging; Twitter; Convolutional Neural Networks}
...
Social media is becoming increasingly multilingual.
...
%\fixme{Others operate at the encoding level}
but our UnicodeCNN can take any Unicode string as input and therefore works with all languages.
%\fixme{We show in particular that the UnicodeCNN understands the concepts of verb conjugations and transliterations between alphabets that are much more important in non-English languages.}
...
%\fixme{We apply to this problem because there are no other large scale labeled NLP problems to train a multilingual feature generator on.}
...
but they explicitly ``do not consider under-represented languages'' due to the difficulty of modeling them.
        \cut{A common feature of all these methods is that they generate features from words in the tweet's text.
...
%The classic work by \citet{han2012geolocation} introduced the widely used method for finding nearby cities to label tweets,
%and claims that predicting GPS coordinates ``presents computational challenges'' and ``is too fine-grained'' for effective prediction. 
...
(See Figure \ref{fig:model} for an example output.)
        \cut{
...
and for understanding differences between African American English and Standard American English \citep{huang2016understanding,gonccalves2017fall,blodgett2016demographic}. %, and the various dialects of Spanish \citep{gonccalves2014crowdsourcing}.
...
We show specific linguistic properties that the UnicodeCNN is sensitive to that no word-level model can detect.
...
Section \ref{sec:model} describes the UnicodeCNN and our novel MvMF output layer for predicting GPS coordinates.
Section \ref{sec:experiments} describes our dataset of 900 million tweets,
baseline comparison models,
and our experimental results.
%We conclude with a discussion about how to apply the UnicodeCNN and MvMF to other domains.
\end{description}
}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        %\node at (0,0) {\resizebox{0.45\textwidth}{!}{\input{img/infer/es-vosotros-hablar/0-good-zoom.pgf}}};
        \node at (0,0) {\resizebox{0.45\textwidth}{!}{\includegraphics{img/infer/es-vosotros-hablar/0-good-zoom}}};
        \node (true) at (-3,-1.2) {\textcolor{black}{\textbf{true}}\\\textcolor{black}{\textbf{location}}};
        \node[shape=circle,inner sep=0,minimum width=0.2cm,fill=black] (dot) at (-0.61,-0.975) {};
        \draw[->,line width=1pt] (true.0) to (dot);
    ...
    \textbf{
    \caption{
        We model a tweet's location as a mixture of von Mises-Fisher distributions over the Earth's surface.
        This map shows the distribution for the tweet \str{No me habl\'eis} from Example 1 below.
        Darker, bluer regions have higher probability, and lighter, greener regions have lower probability.
...
\begin{figure*}
    \centering
    \vspace{-0.25in}
    \resizebox{\textwidth}{!}{\input{fig/model2}}
    \vspace{-0.5in}
    ...
    \caption{
        ...
        %We use two outputs for the geolocation task:
        %the standard cross entropy to predict the country,
        We also introduce the \defn{mixture of von Mises-Fisher} (MvMF) distribution to predict the exact GPS coordinates of a tweet.
	\label{fig:model}
    }
    }
\end{figure*}
...
%\end{quote}
    ...
    \end{quote}
Figure \ref{fig:model} shows the GPS coordinates that our model predicts this tweet was sent from,
...
%,
%and it is reasonable to expect they could be built for Spanish as well since it is the second most widely spoken language in the world.
%Because these methods operate on word level features,
%they need a lexical analyzer/tokenizer that already knows about the different 
...
%The UnicodeCNN does not require explicit tokenization,
...
\begin{figure*}
    ...
    \caption{
        ...
        We use two outputs for the geolocation task:
        the standard cross entropy to predict the country,
        and a novel mixture of von Mises-Fisher distributions to predict the GPS coordinates.
    \label{fig:model}
    }
    }
\end{figure*}
...
The input text is passed to the UnicodeCNN, which generates features in four stages:
a character encoder,
...
The CLCNN was designed only for English language text, however,
and the character encoder and language estimator are the key improvements which let the UnicodeCNN support all languages.
%}{
    %The convolutional layers and feature mixing layers use standard deep learning techniques,
    %and the character encoder and language estimator are the key improvements which let the UnicodeCNN support all languages.
%}
...
For our geolocation application we use a standard cross entropy layer to estimate the country and our novel \defn{mixture of von Mises-Fisher} (MvMF) layer to estimate the GPS location.
%\cut{
Other output layers can be used for other applications, % \citep{wang2015transfer}.,
...
This information represents the combined expertise of hundreds of linguistic experts working for 30 years.
...
Working with Unicode encodings is simple but has two disadvantages.
...
This property facilitates the UnicodeCNN's robustness to spelling mistakes and the transfer of language knowledge observed in Section \ref{sec:experiments}'s qualitative experiments .
}
...
and taking the remainder mod 16.
...
\cutt{
    ...
    the original paper on CLCNNs \citep{zhang2015character} for mathematical details.
    ...
    The widths for each convolutional layer are shown in the table below:
...
%The convolutional layers are a key comp

%We chose to base our convolutional layers on the CLCNN model \citep{},
%but other reasonable alternatives exist as well.

It is possible to replace the CLCNN convolutional layers we use with alternative architectures designed for character-level processing.
Alternative architectures have proposed using deeper convolutional layers with resnet connections \citep{conneau2017very}, 
recurrent neural networks \citep{chung2016character}, 
or complex combinations of convolutional and recurent networks \citep{kim2016character,jozefowicz2016exploring}.
Each of these techniques requires considerable processing resources, however, 
so we do not exhaustively compare these techniques against each other.
(Training the Huge UnicodeCNN requires an estimated 90 GPU-weeks as described in Section \ref{sec:experiments},
and the alternative models are similarly computationally expensive.)
We chose to base our model off of the CLCNN's convolutional layers because it had the best performance on initial tests using a small held-out subset of tweets.
We speculate that convolutions perform better than recurrent networks on twitter analysis because tweets are guaranteed to be short,
but recurrent networks are designed for arbitrary length inputs.
...
The purpose of these convolutional layers is to find important patterns in the input encoding matrix.
Temporal convolution is a standard deep learning method,
and we refer the reader to the \emph{Deep Learning Book} \citep{Goodfellow-et-al-2016} and 
the original paper on CLCNNs \citep{zhang2015character} for mathematical details.
...
%They are formed by convolving a suitable \defn{filter} with the input matrix, 
%then optionally performing \defn{max-pooling}.
There are three important parameters for each convolutional layer: 
the number of channels, the kernel width, and (optionally) the max-pooling width.
%The number of channels determines the number of filters that are applied in parallel,
%each creating its own set of output features.
Increasing the number of channels increases the model's ability to detect important linguistic features,
but it also increases the computational complexity of the model and the model's ability to overfit the data.
...
%It is possible to replace the CLCNN convolutional layers we use with alternative architectures designed for character-level processing.
Many alternatives to the CLCNN have been proposed for character level deep learning models \citep[e.g.][]{conneau2017very,chung2016character,kim2016character,jozefowicz2016exploring}.
We chose to base our model off of the CLCNN's convolutional layers because it is relatively fast and had the best performance on initial tests using a small held-out subset of tweets.
...
\item[Language Encoder.]
	...
	Many systems have been developed that automatically label the language of text \citep[e.g.][]{lui2012langid,jurgens2017incorporating,kocmi2017lanidenn,zhang2018fast},
    ...
    and so tweets written in Croatian get correctly classified as \str{Unknown}.
    But tweets written in Catalan get classified as Spanish,
...
Our model has three outputs: 
the predicted language,
the predicted country of origin,
and the predicted GPS coordinates.
Using multiple related outputs is commonly called \defn{multitask learning} and is a well known technique for accelerating the training of language models \citep[e.g.][]{zhang2017survey,
maurer2016benefit,
kumar2012learning,
romera2012exploiting,
dong2015multi,
caruana1997multitask,
bingel2017identifying}.
The total loss of our model is the sum of the losses for each output.
The loss for the language and country of origin are standard softmax cross entropy losses,
and we introduce a novel loss based on the mixture of von Mises-Fisher distribution to predict GPS coordinates.
...
\cut{
        ...
        %While the mixture of vMF distributions has previously been used in deep learning for clustering \citep{gopal2014mises} and facial recognition \citep{hasnat2017mises},
        %we believe we are the first to apply it to predicting GPS coordinates.

        %The optimization of the mixture of vMF distributions is closely related to optimization of  a mixture of Gaussians, % \citep{variani2015gaussian},
        %except the Gaussian density is replaced by the vMF density.
        ...
        %and each mixture distribution $i$ has its own mean direction $\mu_i$ and concentration parameter $\kappa_i$, 
        ...
        Figure \ref{fig:model} plots the density of an example distribution.
...
\caption{
    ...
    The baseline \str{lang+time+bow} model generalizes methods from previous work \citep{cheng2010you,li2012towards,han2013stacking,mahmud2014home,compton2014geotagging,zhang2014geocoding,rahimi2015twitter,dredze2016geolocation,rahimi2017neural} to our setting of multilingual input and GPS location output.
...
In particular, we use feature hashing \citep{weinberger2009feature} with $2^{22}$ hash values and L1 regularization to automatically select the best features 
...
Our results are not directly comparable to the results in these papers because we consider a harder geolocation problem:
we perform no filtering of hard tweets in our dataset,
...
%\fixme{discuss more complicated NLP methods not used for Twitter.
%Emphasize that the BOW models are simple, but they nevertheless represent the state of the art.
%CLCNN model compared against other NLP methods.}
...
and so the total amount of computation is reduced by almost half in the online setting.
Second, online evaluation is a natural model for tweet geolocation.
New tweets are constantly being written,
and online evaluation lets us easily take advantage of this new data.
...
\cutt{
The character encoding is computed on the CPUs, 
and all other parts of the model are computed on the GPUs.
We use data parallelism, so that each GPU processes 100 tweets per batch, 
...
and in total, training the large model took about 4 weeks with this setup.
}
In total, training the large model took about 4 weeks with this setup.
The results reported for the huge model are on approximately 20\% of the data after 3 weeks of training.
We estimate that training the huge model on the full dataset would take about 15 weeks.
We expect the huge model's performance would continue to slightly improve given more training time.
An ablative study comparing all possible character encodings and model architectures discussed in Section \ref{sec:model} is clearly infeasible when model training is so expensive.
%We did observe, however, that the language and country outputs were necessary for the model to converge in a reasonable time.
...
We observed that the UnicodeCNN model understands verb conjugations and transliterations,
which are both linguistic features important in non-English languages.
We demonstrate these properties by example.

Spanish has a complex system of verb conjugations,
and different regional dialects use different conjugations.
For example, the Spanish words \str{apres\'eis} and \str{apresen} both translate to \str{arrest}, 
but the word \str{apres\'eis} is in the vosotros form 
(which is only used in the Castillian dialect of Spain),
and the word \str{apresen} is in the ustedes form
(which is used in the American dialects).
Neither of these words appears in our training data,
but our model is given these words as input,
it correctly locates \str{apres\'eis} in Spain and \str{apresen} in the Americas.
Spanish is the third most popular language on Twitter with approximately 60 million Spanish tweets sent per day.
But no previous work on tweet geolocation has been able to take advantage of these conjugations.

Many proper names are the same in different languages,
but transliterated into different scripts.
For example, the country Kuwait is written in Arabic as \foreignlanguage{arabic}{الكويت},
Greek as \foreignlanguage{greek}{κουβέιτ},
Russian as \foreignlanguage{russian}{Кувейт},
and Japanese as \begin{CJK}{UTF8}{min}クウェート\end{CJK}.
    Only the English and Arabic versions are present in our training data.
    Nonetheless,
    our UnicodeCNN model recognizes that all these words mean Kuwait.
    For example, the model predicts that text
    \begin{quote}
        I'm in \foreignlanguage{greek}{κουβέιτ}.
    \end{quote}
    is located in Kuwait.
    No previous methods of geolocation are able to take advantage of these transliterations.
\end{description}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{table}
    \input{img/tables/versus2}
    \textbf{
    \caption{
        The performance of the best baseline and UnicodeCNN models across the ten most popular languages as labelled by the Twitter API.
        The Huge UnicodeCNN significantly outperforms the \str{lang+time+bow} baseline model on all languages.
        Notice the dramatic improvement for tweets with unknown language.
        \label{table:results:lang}
    }
    }
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Researchers have explored geolocating many types of data besides tweets,
%including 
%Many deep learning 
%Besides geolocation, many other tasks require predicting GPS coordinates.
%For example:
%Many other tasks
%image geolocation \citep{hays2008im2gps,crandall2009mapping,weyand2016planet},
%forensic biology \citep{arenas2017forensic,clarke2017integrating},
%and ip address geolocation \citep{wang2011towards}.
%The MvMF output layer may prove useful for these applications as well.
