...
%\usepackage[vietnam,russian]{babel}
...
\begin{abstract}
    ...
    The key to our method is a novel character-level \defn{convolutional neural network} (CNN) that accepts any Unicode string as input.
    ...
    We show that our method significantly outperforms baseline best-practices when working with English-only text.
    A disadvantage of our CNN approach is that deep neural networks are notoriously difficult to interpret compared to simpler linear models.
    We mitigate this disadvantage by introducing a novel method for visualizing the activations of our CNN,
    and we use this visualization to discover a number of dialectical differences between geographical regions.
...
but our analysis of tweets sent in 2018 shows that only 32\% of tweets were written in English.
...
These non-English communities are poorly served by state-of-the-art text analysis tools,
which work well only for English and similarly popular languages.
...
We believe the UnicodeCNN is the first model designed to work on all languages.
A major advantage of the UnicodeCNN is that it is naturally robust to the spelling mistakes and non-standard grammar commonly found in social media.
...
Each of these single-language models must be developed from scratch,
and \citet{han2014text} only support the most popular languages on Twitter.
...
We instead solve a variational inference problem using a mixture of Fischer-von Mises distributions to predict the exact GPS location of tweets.
...
\fixme{\~n doesn't appear in tweet.}}
\end{quote}
...
For example, consider the Vietnamese character \str{Ớ}.
...
(e.g.\ \str{Ớ} is transliterated into \str{O}),
...
For example, the \str{U+1EDA} code point (\str{Ớ}) is decomposed into the Latin \str{O} and the two marks \str{U+031B} and \str{U+0301}.
...
\begin{description}
    ...
    \item[Country.]
        ...
        The number of classes is larger than the number of countries recognized by the United Nations
    ...
    \item[GPS Coordinates.]
        ...
        \end{equation}
        where $\kappa\in\R$ is called the \defn{concentration parameter},
        $\mu\in\mathbb{S}^2$ is called the \defn{mean direction},
        and $\x$ is any point in $\mathbb{S}^2$,
        and $\mathbb{S}^2 = \{ \x \in \R^3 : \ltwo{\x}=1 \}$ is the unit sphere.
        The overall loss is then
        \begin{equation}
            -\log\sum_{i=1}^q \omega_i \text{FvM}(\x;\kappa_i,\mu_i),
        \end{equation}
...
Because our dataset is so large and our model so complicated,
we require a non-standard evaluation method.
The standard practice for evaluating models is to split the dataset into training and testing subsets.
An optimizer runs multiple passes over the training set,

\begin{description}
    \item[Training our model.]
We use a single machine with 16 CPUs, 64 GB of RAM, and 6 NVidia Titan x80 GPUs.
We parallelize the model using data parallelism \citep{},
so that each GPU receives minibatches of 100 tweets,
and then the gradient computations are averaged,
so in total the batch size is 600.
We train our model using the Adam optimizer \citep{kingma2014adam} with a learning rate of $5\times10^{-4}$.

\item[Training the baselines.]
The baseline models require significantly less computation.
