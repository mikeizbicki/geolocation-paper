...
%\title{Geolocating Tweets in any Language at any Location\\with Unicode Convolutional Neural Networks\\and Mixtures of von Mises-Fisher Distributions}
...
\begin{abstract}
    ...
    %To predict GPS locations, we introduce a novel output layer for neural networks we call the mixture of von Mises-Fisher (vMF) layer.
    ...
    %a property ignored in all previous geolocation work.
    %whereas all previous geolocation work has ignored the Earth's geometry.
    %We combining the UnicodeCNN and the MvMF,
    ...
    Remarkably, the UnicodeCNN can learn geographic knowledge in one language and automatically transfer that knowledge to other languages.
    ...
    %we are able to predict the exact GPS coordinates that tweets were sent from using a novel mixture of von Mises-Fisher distributions.
...
%Prior work has focused only on English messages,
%but this focus leaves minority communities underserved 
...
\begin{figure}
    ...
    %The probability distribution of where our model locates the tweet in Example 1 below.
        ...
        This map shows the distribution for the tweet \str{No me hablen} from Example 1 below.
        Darker, bluer regions have higher probability, and lighter, greener regions have lower probability.
        %Notice that most of the probability mass is concentrated in Spain (the tweet's country of origin),
...
\begin{description}
    ...
    \item[Mixture of von Mises-Fisher Distributions.]
        ...
        %In this section, 
        %we introduce a novel output layer for our network called the \defn{Mixture of Fischer-von Mises distributions} (MvMF layer).
        %In order to predict the exact GPS coordinates, 
        %we use the mixture of Fisher-vom Mises distributions (MvMF) layer.
        %The MvMF layer outputs a probability distribution over the earth's surface for each tweet.
        %A point estimate of the tweet's location can be computed using the maximum likelihood.

        %The MvMF layer is inspired by the field of \defn{directional statistics},
