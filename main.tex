...
We show specific linguistic properties that the UnicodeCNN is sensitive to that no word-level model can support.
...
Section \ref{sec:model} describes the UnicodeCNN and our novel mixture of von Mises-Fisher layer for predicting GPS coordinates.
...
It is possible to replace the CLCNN convolutional layers we use with alternative deep learning architectures designed for character-level processing.
...
(Training the Huge UnicodeCNN requires an estimated 90 GPU-months as described in Section \ref{sec:experiments},
...
\begin{table}
    ...
    \textbf{
    \caption{
        Ten most popular languages as labelled by the Twitter API.
